...
async function test() {
  ...
  console.log("Testing keyword search for 'solicitors'...");
  const result = await provider.search({ keyword: "solicitors", limit: 10 });
  ...
  if (result.businesses.length > 0) {
    ...
    result.businesses.slice(0, 3).forEach((b, i) => {
      ...
      console.log(`     ${b.address || "No address"}`);
      console.log(`     Phone: ${b.telephone || "N/A"}`);
    });
  }
}
