import { searchPlaces, getPlaceDetails } from '../lib/google-places.ts';
import { extractEvidence } from '../lib/evidence-extractor.ts';
import { generateQuestions, prioritizeQuestions } from '../lib/question-engine.ts';
import { generateProposal } from '../lib/proposal-engine.ts';

async function validateSystem() {
  console.log('🚀 [Validation] Starting system validation with 10 florist businesses\n');

  // For now, use the mock data from google-places.ts
  // In production, this would fetch real data from Google Places API

  const mockBusinesses = [
    { place_id: 'mock_1', name: 'Rose & Thorn Florists' },
    { place_id: 'mock_2', name: 'Bloom & Co' },
    { place_id: 'mock_3', name: 'Petals & Stems' },
    { place_id: 'mock_4', name: 'The Flower Room' },
    { place_id: 'mock_5', name: 'Petal & Co' },
    { place_id: 'mock_6', name: 'Floral Designs Manchester' },
    { place_id: 'mock_7', name: 'Bunches Manchester' },
    { place_id: 'mock_8', name: 'Flower Street' },
    { place_id: 'mock_9', name: 'Stems & Petals' },
    { place_id: 'mock_10', name: 'Manchester Flowers' },
  ];

  console.log(`✓ Found ${mockBusinesses.length} businesses\n`);
  ...
  for (let i = 0; i < mockBusinesses.length; i++) {
    ...
    const reviewSnippets = (details.reviews || []).map((review, idx) => ({
      id: `review_${i}_${idx}`,
      text: review.text,
      source: 'google',
