 ...
 *
 * This script tests the complete workflow journey:
 * Business → Investigation → Conversation → Outcome → Timeline → Audit
 *
 * It measures:
 * - API response times
 * - Data availability for each step
 * - Friction points (missing data, errors)
...
async function testStep(
  name: string,
  url: string
): Promise<Omit<TestResult, "step" | "issues">> {
...
async function runTests() {
  ...
  console.log(
    `✓ Displays: Unreviewed businesses list with review counts\n`
  );
  ...
  console.log(`✓ API responding: ${result.responseTime}ms`);
  console.log(
    `✓ Displays: Outcome signals, truth levels, classifications\n`
  );
  ...
  results.push({
    ...
    issues:
      result.status !== "OK"
        ? [`API Error: ${result.status}`]
        : ["Need to query with actual assumption ID for full test"],
  });
  console.log(`✓ API responding: ${result.responseTime}ms`);
  ...
  console.log(`\nAPIs are working. Now test in browser:\n`);
